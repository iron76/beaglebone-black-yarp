...
export MY_CODE_DIR=$1
    ...
    mkdir -p build
    cd build
cmake .. -DCMAKE_INSTALL_PREFIX:PATH=$MY_CODE_DIR/bin -DICUB_INSTALL_WITH_RPATH:STRING=ON -DENABLE_icubmod_canmotioncontrol:STRING=ON -DENABLE_icubmod_socketcan:STRING=ON -DENABLE_icubmod_xsensmtx:STRING=ON -DENABLE_icubmod_analogServer:STRING=ON -DENABLE_icubmod_sharedcan:STRING=ON -DENABLE_icubmod_canBusAnalogSensor:STRING=ON
    make
    make install
else
    echo "*****************************************************************"
